...
def write_judgments(path: Path, judgments: list[dict]) -> dict:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Classify a screen tree with one local WildGuard model load."
    )
    ...
    summaries = {}
    for arm, group, input_path in files:
        ...
        if [row["id"] for row in sorted(judgments, key=lambda row: row["id"])] != [
            row["id"] for row in rows
        ]:
            raise ValueError(f"Judgment IDs or order differ for {arm}/{group}.")
        summary = write_judgments(
            args.output_root / arm / f"{group}.jsonl", judgments
        )
