...
def mtp_root(model: Any) -> Any:
    candidates = [getattr(model, "mtp", None), getattr(getattr(model, "model", None), "mtp", None)]
    for candidate in candidates:
        if candidate is not None and hasattr(candidate, "layers"):
            return candidate
    raise RuntimeError("The plan includes MTP, but the live model has no mtp.layers module.")


def validate_live_model(model: Any, cfg: ExperimentConfig) -> None:
    backbone = text_backbone(model)
    if len(backbone.layers) != cfg.model.num_layers:
        raise RuntimeError(f"Expected {cfg.model.num_layers} text layers, found {len(backbone.layers)}.")
    ...
    for index, layer in enumerate(backbone.layers):
        ...
        if tuple(down.shape) != (cfg.model.hidden_size, cfg.model.intermediate_size):
            raise RuntimeError(f"Unexpected MLP output shape at layer {index}: {tuple(down.shape)}")
        if index % 4 == 3:
    ...
    if cfg.edit.include_mtp:
        ...
        if tuple(mtp_layer.self_attn.o_proj.weight.shape) != expected_mixer:
            raise RuntimeError("MTP attention output shape does not match the frozen architecture.")
        if tuple(mtp_layer.mlp.down_proj.weight.shape) != expected_down:
            raise RuntimeError("MTP MLP output shape does not match the frozen architecture.")
...
def apply_edit(model: Any, cfg: ExperimentConfig, direction: Any) -> list[str]:
    ...
    for index in range(cfg.edit.first_layer, cfg.edit.last_layer + 1):
        ...
        if cfg.edit.include_mlp_output:
            project_output_weight_(layer.mlp.down_proj.weight, direction, cfg.edit.alpha)
            edited.append(f"model.language_model.layers.{index}.mlp.down_proj.weight")
    if cfg.edit.include_mtp:
        mtp_layer = mtp_root(model).layers[0]
        if cfg.edit.include_attention_output:
            project_output_weight_(mtp_layer.self_attn.o_proj.weight, direction, cfg.edit.alpha)
            edited.append("mtp.layers.0.self_attn.o_proj.weight")
        if cfg.edit.include_mlp_output:
            project_output_weight_(mtp_layer.mlp.down_proj.weight, direction, cfg.edit.alpha)
            edited.append("mtp.layers.0.mlp.down_proj.weight")
    return edited


def capture_last_token_activations(model: Any, processor: Any, prompts: list[str], layer_index: int) -> list[Any]:
    ...
    try:
        for prompt in prompts:
            ...
            batch = {name: value.to(device) for name, value in batch.items() if hasattr(value, "to")}
            captured.clear()
            with torch.inference_mode():
                model(**batch, use_cache=False)
            if len(captured) != 1:
                raise RuntimeError(f"Expected one layer activation, captured {len(captured)}.")
            position = int(batch["attention_mask"][0].sum().item()) - 1
            results.append(captured[0][0, position].float().cpu())
    finally:
        handle.remove()
    return results
